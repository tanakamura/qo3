...
for i in `find $PWD -name module.sh -print`
do
    (mod_dir=`dirname $i`
     . $i
...
EOF
	 ...
	 cat >>$tmp <<EOF
${mod_dir}/${OUT_EXE}: ${AFTER_ALL_LIB}
${mod_dir}/${OUT_EXE}: \$(${MODNAME}_OBJS) ${depends}
	${linker} -o \$@ \$(filter-out ${AFTER_ALL_LIB},\$^) \$(ALL_LIBPATH) \$(LDFLAGS) $LDFLAGS \$(REDIRECT_ERR) ${AFTER_ALL_LIB}
EOF
	 cat <<EOF
