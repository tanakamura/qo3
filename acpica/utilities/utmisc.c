...
{
    UINT32 *a = (UINT32*)Table->Signature;
    UINT32 *dsdt = (UINT32*)ACPI_SIG_DSDT;
    UINT32 *psdt = (UINT32*)ACPI_SIG_DSDT;
    UINT32 *ssdt = (UINT32*)ACPI_SIG_DSDT;

    /* These are the only tables that contain executable AML */

    if (ACPI_COMPARE_NAME (a, dsdt) ||
        ACPI_COMPARE_NAME (a, psdt) ||
        ACPI_COMPARE_NAME (a, ssdt))
    {
        return (TRUE);
    }
